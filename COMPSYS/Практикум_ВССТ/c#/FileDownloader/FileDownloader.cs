	...
	public class FileDownloader
	{
		static string TEMP_DIR
		{
			get
			{
				...
				localTemp = System.IO.Path.Combine(localTemp, "Закачивальщик файлов");
		...
		private void SaveToFile(byte[] buffer, int count, string fileName)
		{
			...
			catch (ArgumentException e)
			{
				throw new ArgumentException(
					String.Format("Ошибка при сохранении файла \"{0}\": {1}", fileName, e.Message), e);
			}
			finally
			{
				if (f != null)
		...
		public static DownloadData Create(string url, string destFolder)
		{
			...
			catch (Exception e)
			{
				throw new ArgumentException(String.Format(
					"Ошибка закачки \"{0}\": {1}", url, e.Message), e);
			}
			...
			if ((response.ContentType.IndexOf("text/html") > -1) || response.StatusCode == HttpStatusCode.NotFound)
			{
				...
				throw new ArgumentException(
					String.Format("Невозможно зхакачать \"{0}\" - веб-страница передана сервером.",
					url));
			}
