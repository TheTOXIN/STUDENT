...
void __fastcall TForm1::Button1Click(TObject *Sender)//NEW
{
	...
	StringGrid1->Cells[2][0]="Пункт";
	StringGrid1->Cells[3][0]="Отправление";
	StringGrid1->Cells[4][0]="Прибытие";
	Button5Click(Sender);
...
void __fastcall TForm1::Button6Click(TObject *Sender)//SORT
{
	...
	for(int i = 1; i < row; i++)
	{
		...
		str_cells[4][i] = StringGrid1->Cells[4][i];
	}
	//удаляем ненужные элементы элементы
	for (int i = 1; i < row; i++) 
	{
		AnsiString cell_date = str_cells[4][i];
		AnsiString date = Edit1->Text;
		TDateTime dt1;
		TDateTime dt2;
		if (TryStrToDateTime(cell_date,dt1) && TryStrToDateTime(date,dt2))
		{
			if (cell_date > date)
			{
				str_cells[1][i] = "";
