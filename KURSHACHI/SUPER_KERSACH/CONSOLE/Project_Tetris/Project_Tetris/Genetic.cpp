...
{
	DNA parent0 = populations[rand() % countDNA];
	DNA parent1 = populations[rand() % countDNA];

	for (int i = 0; i < countDNA; i++)
	{
		int numberGroup = rand() % 2;
		if (numberGroup == 0)
		{
			if (populations[i].fitness > parent0.fitness)
				parent0 = populations[i];
		}
		if (numberGroup == 1)
		{
			if (populations[i].fitness > parent1.fitness)
